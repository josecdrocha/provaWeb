...
async function loadNews() {
    ...
    newsList.innerHTML = newsData?.items?.map(item => {
        const images = JSON.parse(item?.imagens || '{"image_intro": ""}');
...
async function getNews() {
    const url = `https://servicodados.ibge.gov.br/api/v3/noticias${window.location.search}`;
    try {
...
function mountCard(img, title, intro, tags, time, link) {
    return `
        <li class="card">
            <img src="https://agenciadenoticias.ibge.gov.br/${img}" alt="imagem da noticia" class="card-image">
            <div class="card-content">
                <h2 class="card-title">${title}</h2>
                <p class="card-intro">${intro}</p>
                <div class="card-tags">
                    <div class="card-markups">${getTags(tags)}</div>
                    <span class="card-time">${getTime(time)}</span>
                </div>
...
function getTime(time) {
    ...
    if (quantidadeDias === 0) return 'Publicado Hoje';
    if (quantidadeDias === 1) return 'Publicado Ontem';
    return `Publicado ${quantidadeDias} dias atrás`;
}

function calculateTime(time) {
    const formattedTime = time.split(' ')[0].split('/').reverse().join('-');
    const milissegundosTotal = new Date() - new Date(formattedTime);
...
function setFilterQuantity() {
    ...
    if (quantidadeOption !== '10') filterQuantity++;

    const tipoOption = document.querySelector('#tipo').value;
    if (tipoOption !== 'none') filterQuantity++;

    if (document.querySelector("#de").value) filterQuantity++;
    if (document.querySelector("#ate").value) filterQuantity++;
...
function setFilters(event = null) {
    ...
    tipo.value !== 'none' ? url.searchParams.set('tipo', tipo.value) : url.searchParams.delete('tipo');
    dataInicial.value ? url.searchParams.set('de', dataInicial.value) : url.searchParams.delete('de');
    dataFinal.value ? url.searchParams.set('ate', dataFinal.value) : url.searchParams.delete('ate');
...
function loadFilters() {
    ...
    if (quantidadeOption) quantidadeOption.selected = true;

    const tipoOption = Array.from(document.querySelector('#tipo').options).find(p => p.value === tipo);
    if (tipoOption) tipoOption.selected = true;
...
function getPaginationItemHTML(counter, isActive) {
    return `<li><button onclick="setPage(this)" class="${isActive ? 'active' : 'inactive'}">${counter}</button></li>`;
}
